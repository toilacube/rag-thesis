...
interface CitationPayloadEvent { type: "citation_payload"; data: string; } // data is base64 string
...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || `http://${process.env.NEXT_PUBLIC_BACKEND_SERVER || 'localhost'}:${process.env.NEXT_PUBLIC_BACKEND_PORT || '8000'}/api`;
...
export default function ChatInterfacePage() {
  ...
  const [selectedCitation, setSelectedCitation] = useState<CitationContext | null>(null);
  ...
  const copyToClipboard = useCallback(async (text: string, messageIdOrContent: string) => {
    try {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement> | React.KeyboardEvent<HTMLInputElement>) => { // Adjusted type for onKeyPress
    e.preventDefault();
      ...
      while (true) {
        ...
        if (done) {
            ...
            if (streamingAssistantMessage && streamingAssistantMessage.content.trim()) {
              setMessages(prev => [...prev, { ...streamingAssistantMessage, isStreaming: false, tempId: undefined }]);
            }
            setStreamingAssistantMessage(null);
        ...
        while ((eolIndex = buffer.indexOf('\n\n')) >= 0) {
          ...
          if (eventString.startsWith("data: ")) {
            try {
              ...
              if (jsonData.type === "user_message_saved") {
                  ...
                  try {
                      ...
                      if (parsedData.context && Array.isArray(parsedData.context)) {
                          ...
                          console.warn("Received citation_payload with invalid structure:", parsedData);
                          toast({ title: "Citation Warning", description: "Received malformed citation data from server.", variant: "default" });
                      }
                  } catch (error) {
                      console.error("Error decoding/parsing citation_payload:", error, "Raw data:", jsonData.data);
                      toast({ title: "Citation Error", description: "Could not process citation data.", variant: "destructive" });
                  }
              ...
              console.error("Error parsing SSE JSON:", parseError, "Raw event string:", eventString);
            }
  ...
  const handleCitationClick = (citationNumber: number) => {
    ...
    if (citationContexts && index >= 0 && index < citationContexts.length) {
      ...
      console.warn(`Clicked citation [${citationNumber}], but no corresponding context found.`);
      toast({ title: "Citation Not Found", description: `Details for citation ${citationNumber} are not available.`, variant: "default" });
    }
  };

  const renderMessage = (message: Message, isLiveStreaming: boolean = false) => {
    const messageKey = message.tempId || `${message.role}-${message.id || crypto.randomUUID()}`; // Ensure key even if id is 0
    const displayId = !message.tempId && message.id !== 0 ? message.id.toString() : (message.tempId ? '(sending)' : '(streaming)');
    ...
    const messageContent = (
      ...
      >
        {message.role === "assistant" || (isUser && message.content.includes("```")) ? (
          <ReactMarkdown
            ...
            components={{
              a: ({ node, href, children, ...restProps }) => {
                ...
                if (isCitationLink) {
                  ...
                  const index = citationNumber - 1;
                  const contextForTooltip = (citationContexts && index >= 0 && index < citationContexts.length)
                    ? citationContexts[index]
                    : null;
                  
                  const { style } = restProps as React.HTMLAttributes<HTMLElement>;
                    ...
                    <TooltipProvider delayDuration={100}>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            style={style}
                            className={`text-blue-600 dark:text-blue-400 hover:underline focus:outline-none focus:ring-1 focus:ring-blue-500 rounded-sm px-0.5 py-0 align-super text-xs ${(restProps as any).className || ''}`}
                        ...
                        {contextForTooltip && (
                          <TooltipContent className="max-w-xs bg-background text-foreground border shadow-lg rounded-md p-2">
                            <p className="text-xs font-medium">
                              Source: {contextForTooltip.metadata.file_name || "N/A"}
                            </p>
                            {contextForTooltip.metadata.headers && Object.keys(contextForTooltip.metadata.headers).length > 0 && (
                               <p className="text-xs text-muted-foreground mt-1">
                                  {Object.values(contextForTooltip.metadata.headers).join(' > ')}
                               </p>
                            )}
        ...
        {(isLiveStreaming || message.isStreaming) && (
         ...
         <div className="text-xs mt-1 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2"
          ...
          {!isUser && !message.tempId && message.id !== 0 && (
              <Button variant="ghost" size="icon" className="h-5 w-5 p-0" onClick={() => copyToClipboard(message.content, message.id.toString())}>
    ...
    return (
      <div
        key={messageKey}
        className={`flex items-end space-x-2 group mb-2 ${isUser ? "justify-end" : "justify-start"}`}
      >
        {!isUser && avatar}
        {messageContent}
        {isUser && avatar}
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5rem)] relative bg-muted/20">
      <header className="p-4 border-b bg-background shadow-sm">
        <h1 className="text-xl font-semibold truncate" title={chatSession.title}>
          {chatSession.title}
        </h1>
      </header>
      ...
      <Dialog open={isCitationDialogOpen} onOpenChange={setIsCitationDialogOpen}>
        <DialogContent className="sm:max-w-[625px] bg-background text-foreground border shadow-xl">
          <DialogHeader>
            <DialogTitle>
              Citation: {selectedCitation?.metadata.file_name || "Source Details"}
            </DialogTitle>
            {selectedCitation?.metadata.chunk_id && (
              <DialogDescription className="text-xs">
                Document ID: {selectedCitation.metadata.document_id || "N/A"} | Chunk ID: {selectedCitation.metadata.chunk_id}
              </DialogDescription>
            )}
          </DialogHeader>
          <div className="prose prose-sm dark:prose-invert max-w-none max-h-[60vh] overflow-y-auto py-4 px-1 pretty-scrollbar">
            {selectedCitation ? (
              <>
                {selectedCitation.metadata.headers && Object.keys(selectedCitation.metadata.headers).length > 0 && (
                  <div className="mb-3 p-2 border-l-2 border-primary bg-muted/50 rounded-r-md">
                    <p className="text-xs font-semibold text-primary">
                      In section: {Object.values(selectedCitation.metadata.headers).join(' / ')}
                    </p>
                  </div>
                )}
